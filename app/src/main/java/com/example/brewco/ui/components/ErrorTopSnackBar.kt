...
fun ErrorTopSnackbar(
    ...
    AnimatedVisibility(
        visible = isVisible,
        enter = slideInVertically(initialOffsetY = { -it }) + fadeIn(), // Aparece de arriba hacia abajo
        exit = slideOutVertically(targetOffsetY = { -it }) + fadeOut(), // Desaparece hacia arriba
    ) {
        Box(
            modifier = Modifier
                .fillMaxWidth()
                .padding(16.dp),
            contentAlignment = Alignment.TopCenter // Cambia la alineación al centro superior
        ) {
            Card(
                modifier = Modifier
                    .fillMaxWidth()
